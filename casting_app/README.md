# 🏭 Casting AI - 주조 결함 AI 검사 시스템

## 📋 프로젝트 개요
**딥러닝(EfficientNet-B0) + LLM(Claude Sonnet 4.5)**을 결합한 최신 AI 기반 주조 제품 결함 자동 검사 시스템
...
## 🏗️ 프로젝트 구조 (오케스트레이션 패턴)

```
casting_app/
├── app.py                              # 🎭 Orchestrator (중앙 지휘자)
├── config.py                           # ⚙️ 전역 설정
...
### 3️⃣ **모델 파일 확인**
`models/` 폴더에 `final_efficientnet_b0.pth` 파일이 있는지 확인
...
### 5️⃣ **브라우저 접속**
자동으로 열리는 `http://localhost:8501`에서 사용
...
### 아키텍처 패턴
- **오케스트레이션 패턴**: app.py가 중앙 지휘자 역할
- **모듈화**: 각 클래스는 단일 책임 원칙 준수
- **느슨한 결합**: 독립적인 모듈 간 통신
